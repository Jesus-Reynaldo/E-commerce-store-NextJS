...
export const AddressFrom = ({countries,userStoreAddress={}}:Props) => {
  ...
  const onSubmit = async(data:FormInput)=>{
    ...
    if(rememberAddress){
      await setUserAddress(restAdress,session!.user.id )
    }else{
  ...
  return (
    <form onClick={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-2 sm:gap-5 sm:grid-cols-2">
      ...
      <div className="flex flex-col mb-2">
        ...
        <input type="text" className="p-2 border rounded-md bg-gray-200" {...register('zipCode',{required:true})}/>
      </div>
      ...
      <div className="flex flex-col mb-2 sm:mt-10 gap-2 ">
        <div className="inline-flex items-center mb-2">
          ...
          <span>Redordar direccion</span>
        </div>

        <button
        disabled={isValid}
          type="submit"
          className={clsx({"btn-primary hover:cursor-pointer":isValid, "btn-disabled hover:cursor-default":!isValid,})}
        >
          Siguiente
        </button>
      </div>
